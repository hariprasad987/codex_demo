...
import type { Genre, Movie } from '@/lib/data';
import { useMemo, useState } from 'react';

export default function MoviesClient({ movies, genres }: { movies: Movie[]; genres: Genre[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (minYear) res = res.filter((m) => m.year >= minYear);
    if (maxYear) res = res.filter((m) => m.year <= maxYear);
    res.sort((a, b) => {
    ...
    return res;
  }, [movies, selectedGenres, sort, minYear, maxYear]);
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex flex-wrap gap-4'>
        {paged.map((m) => (
          <Card key={m.id} item={m} type='movie' />
        ))}
      </div>
